...
test("shows the top 100 verified games on the ranking page", async () => {
  ...
  assert.match(source, /実質プレイ費用が安いSwitch 2\/Switchソフト/);
});

test("uses compact Switch 2 hero typography", async () => {
  const styles = await readFile(new URL("../src/styles.css", import.meta.url), "utf8");
  assert.match(styles, /\/\* Switch 2 hero typography \*\/[\s\S]*?\.hero-copy p\{font-size:34px/);
  assert.match(styles, /\/\* Switch 2 hero typography \*\/[\s\S]*?\.hero-copy h1\{font-size:46px/);
});
