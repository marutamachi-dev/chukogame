...
test("selects an explicit chunk and rejects an invalid chunk", () => {
  ...
  assert.equal(selectRefreshChunk("9"), 9);
  assert.throws(() => selectRefreshChunk("10"), /GAME_CHUNK/);
});
...
test("rotates the default chunk deterministically by UTC day", () => {
  ...
  assert.equal(selectRefreshChunk(undefined, next), (selectRefreshChunk(undefined, day) + 1) % 10);
});

test("selects every chunk for a full backfill", () => {
  assert.deepEqual(selectRefreshChunks("all"), Array.from({ length: 10 }, (_, index) => index));
});
