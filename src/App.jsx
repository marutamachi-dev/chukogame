...
function CompareCard({ kind, rows }) { const buying = kind === "buy"; return <article className={`compare-card ${kind}`}><h2>{buying ? "買う先を比較する" : "売る先を比較する"}</h2>{rows.length ? rows.map((row, index) => <div className="compare-row" key={row.name}><div><strong>{row.name}</strong>{index === 0 && <em>{buying ? "最安" : "最高"}</em>}</div><b>{yen(row.price)}</b><a href={row.url} target="_blank" rel="noreferrer">{buying ? "買う" : "売る"}<ExternalLink size={15}/></a></div>) : <div className="empty-state">参照できる情報がありません</div>}<p>{buying ? "在庫がある、送料無料と確認できる通常中古品のみを表示しています。" : "参考買取価格です。店頭・宅配で高い方を表示します。"}</p></article>; }function Detail({ game, onBack, onOpen }) { const cost = playCost(game); return <main><div className="breadcrumb"><button onClick={onBack}>← ホームへ戻る</button></div><section className="detail-hero"><Cover game={game} large/><div><p className="eyebrow">Nintendo Switch / 国内パッケージ版</p><h1>{game.title}</h1><p className="detail-meta">JAN: {game.jan}　ジャンル: {game.genre}</p></div><button className="favorite" title="お気に入り"><Heart size={21}/></button><div className="cost-banner"><span>実質プレイ費用の目安</span><strong>{cost === null ? "算出できません" : yen(cost)}</strong><small>{cost === null ? "売値データを確認できません" : `最安買値 ${yen(cheapestBuy(game))} - 最高売値 ${yen(highestSale(game))}`}</small></div></section><Formula game={game}/><section className="trend-panel"><h2>中古相場の推移</h2><p className="trend-basis">相場推移は、送料込み・通常中古品として確認できる複数販売先の販売価格の中央値を基準に算出します。</p><div className="trend-periods">{["直近7日", "直近14日", "直近28日"].map((period) => <div className="trend-period" key={period}><span>{period}</span><b>価格推移を収集中</b></div>)}</div><p className="trend-note">実測履歴が各期間に満たないため、価格変化は表示していません。</p></section><section className="compare-grid"><CompareCard kind="buy" rows={[...game.purchase].sort((a,b) => a.price-b.price)}/><CompareCard kind="sell" rows={[...(game.sale || [])].sort((a,b) => b.price-a.price)}/></section><section className="related"><h2>よく一緒に見られるソフト</h2><div className="related-ranking-grid">{games.filter((item) => item.id !== game.id).slice(0, 5).map((item) => <button className="related-ranking-item" key={item.id} onClick={() => onOpen(item)}><Cover game={item}/><span className="related-ranking-title">{item.title}</span><strong>{playCost(item) === null ? "算出できません" : yen(playCost(item))}</strong><span className="related-ranking-open">詳細を見る <ArrowRight size={13}/></span></button>)}</div></section><ShareBar/></main>; }
